...
void loadDataFromFile(BST& tree) {
	...
	if (loadFromFile(tree, filename)) {
		...
		cout << "Expected format: program, gNumber, name ,standing" << endl;
	}
}

//Adding a new student to the BST
...
//Editing a student's academic standing
void editStudent(BST& tree) {
	...
	// Retrieve student first to get current info
	...
	if (tree.removeByGNumber(gNumber)) {
		...
		else {//reinsert with old standing if update fails
